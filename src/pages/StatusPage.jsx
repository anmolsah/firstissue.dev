...
const StatusPage = () => {
  ...
  const getStatusStats = () => {
    ...
    statusOptions.slice(1).forEach((status) => {
      stats[status.value] = bookmarks.filter(
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Progress Overview */}
      <div className="bg-gradient-to-r from-red-600 via-orange-600 to-green-600 rounded-2xl p-8 mb-8 text-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {statusOptions.slice(1).map(({ value, label, icon: Icon, color }) => {
          ...
          const colorClasses = {
            ...
            blue: "bg-orange-100 text-orange-700 border-orange-200",
          ...
          return (
            <div
              key={value}
              className={`p-6 rounded-xl border-2 transition-all duration-200 hover:scale-105 cursor-pointer ${
                colorClasses[color]
              } ${selectedStatus === value ? "ring-2 ring-red-500" : ""}`}
              onClick={() =>
